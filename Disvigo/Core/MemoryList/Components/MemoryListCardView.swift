...
struct MemoryListCardView: View {
    let memory: Memory
    let onTapGesture: () -> ()
    ...
    var body: some View {
        Button {
            ...
            .contextMenu(menuItems: {
                DLabelButtonView(systemImage: AppIcons.basket, title: String(localized: "Go to Detail")) {
                    onTapGesture()
                }
            })
...
#Preview {
    VStack {
        MemoryListCardView(memory: DeveloperPreview.shared.memories[0]) {}

        HStack {
            MemoryListCardView(memory: DeveloperPreview.shared.memories[1]) {}
            MemoryListCardView(memory: DeveloperPreview.shared.memories[2]) {}
        }
    }
}
